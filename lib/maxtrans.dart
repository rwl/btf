...
///
/// [k] which stage of the main loop we're in
/// [Ap] column pointers, size n+1
/// [Ai] row indices, size nz = Ap [n]
/// [match] size n,  Match [i] = j if col j matched to i
/// [cheap] rows Ai [Ap [j] .. Cheap [j]-1] alread matched
/// [flag] flag[j] = k if j already visited this stage
/// [Istack] size n.  Row index stack.
/// [Jstack] size n.  Column index stack.
/// [Pstack] size n.  Keeps track of position in adjacency list
/// [work] size 1.  Work performed by the depth-first-search
/// [maxwork] maximum work allowed
int augment(int k, List<int> Ap, List<int> Ai, List<int> match, List<int> cheap,
            List<int> flag, List<int> Istack, List<int> Jstack, List<int> Pstack,
...
///
/// [nrow] A is nrow-by-ncol in compressed column form
/// [ncol]
/// [Ap] size ncol+1
/// [Ai] size nz = Ap[ncol]
/// [maxwork] do at most maxwork*nnz(A) work; no limit if <= 0. This
/// work limit excludes the O(nnz(A)) cheap-match phase.
/// [work] size 1, work = -1 if maxwork > 0 and the total work performed
...
/// Returns the # of columns in the matching.
int maxtrans(int nrow, int ncol, List<int> Ap, List<int> Ai, double maxwork,
             List<double> work, List<int> match) {
  ...
  final cheap = new Int32List(ncol);
  final flag = new Int32List(ncol);

  /* stack for non-recursive depth-first search in augment function */
  final Istack = new Int32List(ncol);
  final Jstack = new Int32List(ncol);
  final Pstack = new Int32List(ncol);
